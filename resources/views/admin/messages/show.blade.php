    ...
    <div class="col-xl-10">
        <div class="card border-0 shadow-lg overflow-hidden" style="background: rgba(30, 30, 30, 0.6); backdrop-filter: blur(15px); border-radius: 1.5rem;">
            <div class="card-header bg-transparent border-0 p-4 d-flex align-items-center justify-content-between" style="border-bottom: 1px solid rgba(255,255,255,0.05) !important;">
                ...
                <div class="badge bg-success bg-opacity-25 text-success border border-success border-opacity-50 px-3 py-2">
                    Active Session
                </div>
            </div>

            <div class="card-body p-0">
                <div id="chat-messages" class="p-4" style="height: 500px; overflow-y: auto; background: rgba(0, 0, 0, 0.1);">
                            ...
                            <div style="max-width: 70%; position: relative;">
                                @if($message->replyTo)
                                    <div class="small text-muted mb-1 px-2 py-1 rounded" style="background: rgba(255,255,255,0.05); border-left: 3px solid #0d6efd; font-size: 0.75rem;">
                                        <div class="fw-bold">{{ $message->replyTo->sender_id === Auth::id() ? 'You' : $message->replyTo->sender->name }}</div>
                                        <div class="text-truncate" style="max-width: 200px;">{{ $message->replyTo->message }}</div>
                                    </div>
                                @endif
                                <div class="p-3 message-bubble {{ $message->sender_id === Auth::id() ? 'bg-primary text-white shadow-lg' : 'bg-dark text-light border border-secondary' }}"
                                     data-id="{{ $message->id }}" data-text="{{ $message->message }}" data-sender="{{ $message->sender_id === Auth::id() ? 'You' : $user->name }}" style="cursor: context-menu; border-radius: 1rem;">
                                ...
                                @endif
                                <small class="text-muted d-block mt-1 {{ $message->sender_id === Auth::id() ? 'text-end' : 'text-start' }}" style="font-size: 0.7rem;">
                ...
                <div class="p-4 border-top border-secondary bg-dark bg-opacity-25">
                    {{-- Reply Banner --}}
                    ...
                    <form action="{{ route('admin.messages.store', $user->id) }}" method="POST" enctype="multipart/form-data" id="messageForm">
                        @csrf
                        ...
                        <div class="input-group">
                            ...
                            <input type="text" name="message" id="messageInput" class="form-control bg-dark border-secondary text-white py-3 px-4 shadow-none"
                                   placeholder="Type your response..." maxlength="5000" style="border-radius: 0; border-left: none;">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        menuDeleteThread.addEventListener('click', function(e) {
            ...
            if(confirm('Are you sure you want to permanently delete this entire conversation?')) {
                fetch('{{ route('messages.thread_destroy') }}', {
                  ...
                  .then(d => { if(d.success) window.location.href = '{{ route('admin.messages.index') }}'; else alert('Error deleting conversation.'); })
        ...
        function deleteMessages(ids) {
            fetch('{{ route('messages.bulk_destroy') }}', {
              ...
              .then(d => { if(d.success) location.reload(); else alert('Error deleting messages.'); })
              .catch(() => alert('Network error.'));
        }
    });
</script>
